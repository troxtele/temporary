jQuery(document).ready(function () {
    ...
    if (tab_id != "") {
      ...
      if (jQuery(tab_id).length > 0) {
        if (
          jQuery(tab_id)
            .find(".uabb-tabs")
            .find(' > .uabb-tabs-nav > ul > li[data-index="' + dataindex + '"]')
        ) {
          var current_li = jQuery(tab_id)
              .find(".uabb-tabs")
              .find(
                ' > .uabb-tabs-nav > ul > li[data-index="' + dataindex + '"]'
              )
              .first(),
  ...
  jQuery(".uabb-tabs")
    .find(" > .uabb-tabs-nav > ul > li")
    .on("mouseover keypress ", function (e) {
      e.preventDefault();
      ...
      parent_uabb_tabs
        ...
        .attr("aria-expanded", "false");
      parent_uabb_tabs
        .find(" > .uabb-content-wrap > .section:eq(" + current_li_index + ")")
        .addClass("uabb-content-current")
        .attr("aria-current", "true")
        .attr("aria-expanded", "true");
  ...
  jQuery(".uabb-tabs")
    .find(" > .uabb-content-wrap > .section > .uabb-tab-acc-title")
    .on("click keypress", function (e) {
      ...
      parent_uabb_tabs
        ...
        .attr("aria-expanded", "false");
      parent_uabb_tabs
        .find(" > .uabb-tabs-nav > ul > li:eq(" + current_tab_acc_index + ")")
        .addClass("uabb-tab-current")
        .attr("aria-current", "true")
        .attr("aria-expanded", "true");
      ...
      if (e.originalEvent !== undefined) {
        current_tab_acc
          .parent()
          .siblings()
          .find(" > .uabb-tab-acc-content:lt(1)")
          .slideUp(300);
        if (
          current_tab_acc
            .siblings(".uabb-tab-acc-content")
            .css("display")
            .toLowerCase() === "none"
        ) {
          current_tab_acc
...
(function ($) {
  ...
  UABBTabs.prototype = {
    ...
    _resizeContent: function () {
      $(this.nodeClass + " .uabb-tabs-layout-vertical").each(
        $.proxy(this._resizeVertical, this)
      );
    },
        ...
        if (jQuery(tab_id).length > 0) {
          if (
            ...
            parent_uabb_tabs
              ...
              .trigger("click");
            var trigger_args =
              ".fl-node-" + adv_node + " .uabb-content-current";
            // Trigger the Adv Tab Click trigger.
            UABBTrigger.triggerHook("uabb-tab-click", trigger_args);
